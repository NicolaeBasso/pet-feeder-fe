...
const Logout = ({ dispatchUserLogout }) => { //create a function, pass in the dispatched key from our mapsDispatchtoProps
    const history = useNavigate(); //convention to make a const variable assigned to useHistory function call

    const handleOnClick = () => {
        dispatchUserLogout().then(() => history('/')) //passing our mapDispatch, get promise. then call push on our history const
    }
